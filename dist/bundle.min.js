const AGENT_COLORS={main:"#3b82f6",exec:"#8b5cf6",coo:"#10b981",cfo:"#f59e0b",cmp:"#ec4899",dev:"#06b6d4",sec:"#ef4444",smm:"#f97316",family:"#84cc16",tax:"#6366f1",docs:"#14b8a6",cto:"#8b5cf6",creative:"#d946ef",forge:"#0891b2",quill:"#0ea5e9",chip:"#22c55e"};let state={status:"idle",model:"opus 4.5",currentTask:null,subagent:null,tasks:{todo:[],progress:[],done:[],archive:[]},notes:[],activity:[],docs:[],pendingNotify:null,live:{status:"idle",task:null,taskStarted:null,thoughts:[],lastActive:null,tasksToday:0},console:{logs:[],expanded:!1},chat:{messages:[]},system:{messages:[]}};const CHAT_KEY_PREFIX="solobot-chat-",SYSTEM_KEY="solobot-system-messages";function chatStorageKey(e){const t=e||(typeof window<"u"?window.currentSessionName:null)||localStorage.getItem("gateway_session")||"agent:main:main";return CHAT_KEY_PREFIX+t}const _sessionMessageCache=new Map;function cacheSessionMessages(e,t){!e||!t||_sessionMessageCache.set(e,t.slice(-100))}function getCachedSessionMessages(e){return _sessionMessageCache.get(e)||null}function loadPersistedMessages(){try{const t=localStorage.getItem(SYSTEM_KEY);if(t){const n=JSON.parse(t);if(Array.isArray(n)){const s=Date.now()-864e5;state.system.messages=n.filter(o=>o.time>s)}}}catch{}const e=chatStorageKey();try{const t=localStorage.getItem(e);if(t){const n=JSON.parse(t);if(Array.isArray(n)&&n.length>0){state.chat.messages=n;return}}}catch{}loadChatFromServer()}async function loadChatFromServer(){try{const e=await fetch("/api/state");if(!e.ok)return;const t=await e.json();t.chat?.messages?.length>0&&(state.chat.messages=t.chat.messages,localStorage.setItem(chatStorageKey(),JSON.stringify(state.chat.messages)),typeof renderChatMessages=="function"&&renderChatMessages(),typeof renderChatPage=="function"&&renderChatPage())}catch{}}function persistSystemMessages(){try{const e=state.system.messages.slice(-30);localStorage.setItem(SYSTEM_KEY,JSON.stringify(e))}catch{}}function persistChatMessages(){try{const e=state.chat.messages.slice(-200),t=chatStorageKey();localStorage.setItem(t,JSON.stringify(e)),cacheSessionMessages(currentSessionName||window.currentSessionName,e),syncChatToServer(e)}catch{}}let chatSyncTimeout=null;function syncChatToServer(e){chatSyncTimeout&&clearTimeout(chatSyncTimeout),chatSyncTimeout=setTimeout(async()=>{try{await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:e})})}catch{}},2e3)}loadPersistedMessages();const GATEWAY_CONFIG={host:localStorage.getItem("gateway_host")||"",port:parseInt(localStorage.getItem("gateway_port"))||443,token:localStorage.getItem("gateway_token")||"",sessionKey:localStorage.getItem("gateway_session")||"agent:main:main",maxMessages:100};function saveGatewaySettings(e,t,n,s){const o=s||"agent:main:main";localStorage.setItem("gateway_host",e),localStorage.setItem("gateway_port",t.toString()),localStorage.setItem("gateway_token",n),localStorage.setItem("gateway_session",o),GATEWAY_CONFIG.host=e,GATEWAY_CONFIG.port=t,GATEWAY_CONFIG.token=n,GATEWAY_CONFIG.sessionKey=o,console.log("[saveGatewaySettings] Saved sessionKey:",o)}let gateway=null,streamingText="",_streamingSessionKey="",isProcessing=!1,lastProcessingEndTime=0,historyPollInterval=null,sessionVersion=0,newTaskPriority=1,newTaskColumn="todo",selectedTasks=new Set,editingTaskId=null,currentModalTask=null,currentModalColumn=null,refreshIntervalId=null,taskModalOpen=!1;const DISABLE_SYSTEM_FILTER=!1;async function loadState(){const e=state.chat,t=state.system;try{const s=await fetch("/api/state",{cache:"no-store"});if(s.ok){const o=await s.json();o.tasks=o.tasks||{todo:[],progress:[],done:[],archive:[]},o.tasks.archive=o.tasks.archive||[],delete o.pendingChat,delete o.chat,state={...state,...o,tasks:o.tasks,activity:o.activity||[],_taskVersion:o._taskVersion||0,chat:e,system:t};const a=(o.tasks.todo?.length||0)+(o.tasks.progress?.length||0)+(o.tasks.done?.length||0);console.log(`[loadState] Loaded: ${a} tasks, v${o._taskVersion||0}`),localStorage.setItem("solovision-dashboard",JSON.stringify(state));return}}catch{}const n=localStorage.getItem("solovision-dashboard");if(n){const s=JSON.parse(n);delete s.system,delete s.console,state={...state,...s,chat:e,system:t}}else initSampleData()}const SYNC_API="/api/sync";async function saveState(e=null){state.localModified=Date.now(),e&&(state.lastChange=e);try{const t=JSON.parse(JSON.stringify(state));t.chat&&t.chat.messages&&(t.chat.messages=t.chat.messages.slice(-200)),t.console&&t.console.logs&&(t.console.logs=t.console.logs.slice(-500)),t.activity&&(t.activity=t.activity.slice(-200)),localStorage.setItem("solovision-dashboard",JSON.stringify(t))}catch(t){t.name==="QuotaExceededError"?(console.warn("[Dashboard] localStorage full, clearing old data..."),localStorage.removeItem("solovision-dashboard"),localStorage.removeItem("solobot-chat"),localStorage.removeItem("solobot-system-messages")):console.error("[Dashboard] saveState error:",t)}updateLastSync(),await syncToServer()}async function syncToServer(){try{try{const n=await fetch("/api/state",{cache:"no-store"});if(n.ok){const s=await n.json(),o=s._taskVersion||0;o>(state._taskVersion||0)&&(console.log(`[Sync] Server tasks newer (v${o} > v${state._taskVersion||0})`),state.tasks=s.tasks||state.tasks,state._taskVersion=o,localStorage.setItem("solovision-dashboard",JSON.stringify(state)),renderTasks())}}catch{}const e=JSON.parse(JSON.stringify(state));delete e.tasks,delete e._taskVersion,delete e.activity,delete e.chat,delete e.system,delete e.console,(await fetch(SYNC_API,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)})).ok&&state.console?.logs&&(state.console.logs.push({text:"State synced to server",type:"info",time:Date.now()}),state.console.logs.length>500&&(state.console.logs=state.console.logs.slice(-500)),renderConsole())}catch(e){console.error("Sync error:",e)}}function initSampleData(){state.tasks={todo:[],progress:[],done:[],archive:[]},state.notes=[],state.activity=[],state.docs=[],saveState()}function initDashboardTasks(){console.log("[Dashboard] Task initialization skipped \u2014 tasks managed server-side")}function formatTime(e){if(!e)return"";const t=new Date(e);return isNaN(t.getTime())?"":t.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})}function formatSmartTime(e){if(!e)return"";const n=Date.now()-e;return n<3e4?"just now":n<6e4?`${Math.floor(n/1e3)}s ago`:n<36e5?`${Math.floor(n/6e4)}m ago`:n<864e5?`${Math.floor(n/36e5)}h ago`:n<6048e5?`${Math.floor(n/864e5)}d ago`:new Date(e).toLocaleDateString("en-US",{month:"short",day:"numeric"})}function formatTimeShort(e){return e?new Date(e).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"}):""}function formatRelativeTime(e){if(!e)return"Unknown";const t=Date.now()-e,n=Math.floor(t/6e4);if(n<1)return"Just now";if(n<60)return`${n}m ago`;const s=Math.floor(n/60);return s<24?`${s}h ago`:formatDate(e)}function formatDate(e){return new Date(e).toLocaleDateString("en-US",{month:"short",day:"numeric"})}function updateLastSync(){document.getElementById("last-sync").textContent=formatTime(Date.now())}function getPriorityClass(e){return e===0?"badge-error":e===1?"badge-warning":"badge-default"}function getPriorityBadgeClass(e){return e===0?"badge-error":e===1?"badge-warning":"badge-default"}function getLogColor(e){switch(e){case"command":return"text-green-400";case"success":return"text-green-300";case"error":return"text-red-400";case"warning":return"text-yellow-400";case"info":return"text-blue-400";case"thinking":return"text-purple-400";case"output":return"text-gray-300";default:return"text-gray-400"}}function getLogPrefix(e){switch(e){case"command":return"$ ";case"thinking":return"\u{1F9E0} ";case"success":return"\u2713 ";case"error":return"\u2717 ";case"warning":return"\u26A0 ";default:return""}}function getDocIcon(e){return getDocIconSymbol(e,"")}function addActivity(e,t="info"){state.activity.push({time:Date.now(),action:e,type:t}),state.activity.length>500&&(state.activity=state.activity.slice(-500))}function updateArchiveBadge(){const e=document.getElementById("archive-badge");if(!e)return;const t=(state.tasks.archive||[]).length;e.textContent=t,t>0?e.classList.remove("hidden"):e.classList.add("hidden")}let confirmResolver=null;function showToast(e,t="info",n=4e3){const s=document.getElementById("toast-container");if(!s)return;const o=document.createElement("div");switch(o.style.cssText=`
    ...
    `).join("")||'<div style="text-align: center; color: var(--text-muted); padding: var(--space-4);">No notes yet</div>'}function renderActivity(){const e=document.getElementById("activity-log");if(!e)return;const t=state.activity.slice().reverse().slice(0,50);if(t.length===0){e.innerHTML='<div style="text-align: center; color: var(--text-muted); padding: var(--space-4);">No activity yet</div>';return}const n=Date.now(),s=new Date().toDateString(),o=new Date(n-864e5).toDateString(),a=n-7*864e5,i={today:[],yesterday:[],thisWeek:[],older:[]},r=[];t.forEach(l=>{const m=new Date(l.time).toDateString();if(/system|heartbeat|audit|auto/i.test(l.action)){r.push(l);return}m===s?i.today.push(l):m===o?i.yesterday.push(l):l.time>a?i.thisWeek.push(l):i.older.push(l)});const c=l=>{const m=formatTime(l.time);let u="\u{1F4CB}",g=escapeHtml(l.action);const p=l.action.toLowerCase();p.includes("completed")||p.includes("done")?u="\u2705":p.includes("started")||p.includes("began")?u="\u25B6\uFE0F":p.includes("created")||p.includes("added")?u="\u2795":p.includes("deleted")||p.includes("removed")?u="\u{1F5D1}\uFE0F":p.includes("updated")||p.includes("edited")?u="\u270F\uFE0F":(p.includes("error")||p.includes("failed"))&&(u="\u274C");const y=l.type==="success"?"success":l.type==="error"?"warning":"";return`
            <div class="activity-item">
                <span style="font-size: 14px; margin-right: 6px;">${u}</span>
                <div style="flex: 1; min-width: 0;">
                    <span class="activity-text ${y}">${g}</span>
                    <span class="activity-time" style="font-size: 10px; color: var(--text-muted); margin-left: 6px;">${m}</span>
                </div>
            </div>
    ...
    `}function generateMiniHeatmap(e,t=4,n=7){const s=[];for(let o=0;o<t*n;o++){const a=e[o]||0,i=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][o%7];s.push(`<div class="heatmap-cell level-${a}" title="${i}: Level ${a}"></div>`)}return`<div class="mini-heatmap" style="grid-template-columns: repeat(${n}, 1fr);">${s.join("")}</div>`}function generateActivityHeatmap(e){return`<div class="activity-heatmap">${e.map((n,s)=>{const o=`${s}:00`;return`<div class="activity-heatmap-cell ${n>0?"active":""}" style="opacity: ${.2+n/5*.8}" title="${o}: Level ${n}"></div>`}).join("")}</div>`}function generateActivityData(){const e=[];for(let t=0;t<28;t++){const n=Math.random();let s=0;n>.6&&(s=1),n>.75&&(s=2),n>.85&&(s=3),n>.93&&(s=4),n>.98&&(s=5),e.push(s)}return e}function generateHourlyActivityData(){const e=[];for(let t=0;t<24;t++){let n=0;t>=9&&t<=17&&(n=2),t>=13&&t<=15&&(n=3);const s=Math.random();let o=n;s>.7&&(o=Math.min(n+1,5)),s>.9&&(o=Math.min(n+2,5)),s<.3&&n>0&&(o=Math.max(n-1,0)),e.push(o)}return e}function updateQuickStats(){const e=state.tasks?.done?.length||0,t=document.getElementById("stat-tasks-done");t&&(t.textContent=e,updateSparklineData("tasks",e));const n=document.getElementById("stat-focus-sessions");n&&(n.textContent=focusTimer.sessions,updateSparklineData("focus",focusTimer.sessions));const s=new Date().toDateString(),o=(state.chat?.messages||[]).filter(d=>new Date(d.time).toDateString()===s).length,a=document.getElementById("stat-messages");a&&(a.textContent=o,updateSparklineData("messages",o)),updateStreak();const i=document.getElementById("stat-streak");i&&(i.textContent=statsState.streak);const r=document.getElementById("stat-uptime");if(r){const d=Math.floor((Date.now()-statsState.sessionStartTime)/6e4);if(d<60)r.textContent=`${d}m`;else{const l=Math.floor(d/60),m=d%60;r.textContent=`${l}h ${m}m`}}const c=document.getElementById("stats-last-updated");c&&(c.textContent="Updated just now"),renderSparklines(),renderHeatmaps(),renderProgressRings()}function renderSparklines(){const e=document.getElementById("sparkline-tasks");if(e){const s=statsState.history.tasks.length>1?statsState.history.tasks[statsState.history.tasks.length-1]-statsState.history.tasks[statsState.history.tasks.length-2]:0,o=s>0?"positive":s<0?"negative":"neutral";e.innerHTML=generateSparkline(statsState.history.tasks,60,24,o);const a=document.getElementById("trend-tasks");a&&(a.textContent=s>=0?`+${s}`:s,a.className=`stat-change ${s>0?"positive":s<0?"negative":""}`)}const t=document.getElementById("sparkline-messages");t&&(t.innerHTML=generateSparkline(statsState.history.messages,60,24,"neutral"));const n=document.getElementById("sparkline-focus");n&&(n.innerHTML=generateSparkline(statsState.history.focus,60,24,"positive"))}function renderHeatmaps(){const e=document.getElementById("activity-heatmap-container");if(e&&!e.dataset.initialized){const n=generateActivityData();e.innerHTML=generateMiniHeatmap(n,4,7),e.dataset.initialized="true"}const t=document.getElementById("hourly-heatmap-container");if(t&&!t.dataset.initialized){const n=generateHourlyActivityData();t.innerHTML=generateActivityHeatmap(n),t.dataset.initialized="true"}}function renderProgressRings(){const e=document.getElementById("progress-ring-tasks");if(e){const n=(state.tasks?.todo?.length||0)+(state.tasks?.progress?.length||0)+(state.tasks?.done?.length||0),s=state.tasks?.done?.length||0;e.innerHTML=generateProgressRing(s,n||1,36,3,"var(--success)")}const t=document.getElementById("progress-ring-daily");if(t){const n=new Date().toDateString(),s=(state.tasks?.done||[]).filter(o=>(o.completedAt?new Date(o.completedAt).toDateString():null)===n).length;t.innerHTML=generateProgressRing(s,10,36,3,"var(--brand-red)")}}function updateStreak(){const e=localStorage.getItem("lastActiveDate"),t=new Date().toDateString(),n=new Date(Date.now()-864e5).toDateString();e!==t&&(e===n?statsState.streak++:e!==t&&(statsState.streak=1),localStorage.setItem("dashboardStreak",statsState.streak.toString()),localStorage.setItem("lastActiveDate",t))}function initSparklineData(){["tasks","messages","focus","activity"].forEach(t=>{const n=localStorage.getItem(`stats_history_${t}`);if(!n||n==="[]"){const s=[];for(let o=0;o<7;o++)s.push(Math.floor(Math.random()*10)+5);localStorage.setItem(`stats_history_${t}`,JSON.stringify(s)),statsState.history[t]=s}})}initSparklineData(),setInterval(updateQuickStats,6e4),window.QuickStats={update:updateQuickStats,generateSparkline,generateProgressRing,generateMiniHeatmap,generateActivityHeatmap};const Sparklines={generatePath(e,t=60,n=24){if(!e||e.length<2)return"";const s=Math.min(...e),a=Math.max(...e)-s||1;return`M ${e.map((r,c)=>{const d=c/(e.length-1)*t,l=n-(r-s)/a*n;return`${d},${l}`}).join(" L ")}`},generateAreaPath(e,t=60,n=24){if(!e||e.length<2)return"";const s=Math.min(...e),a=Math.max(...e)-s||1;return`M ${e.map((r,c)=>{const d=c/(e.length-1)*t,l=n-(r-s)/a*n;return`${d},${l}`}).join(" L ")} L ${t},${n} L 0,${n} Z`},render(e,t,n="neutral"){const s=document.getElementById(e);if(!s||!t||t.length<2)return;const o=60,a=24,i=n==="positive"?"var(--success)":n==="negative"?"var(--error)":"var(--brand-red)",r=n==="positive"?"var(--success)":n==="negative"?"var(--error)":"var(--brand-red)",c=this.generatePath(t,o,a),d=this.generateAreaPath(t,o,a);s.innerHTML=`
        ...
        `;return}try{const n=(await gateway._request("sessions.list",{includeGlobal:!0}))?.sessions||[];renderAgentStatuses(n)}catch(t){console.warn("[Agents] Failed to fetch sessions:",t.message),e.innerHTML='<div style="color: var(--text-muted); font-size: 12px; text-align: center;">Failed to load</div>'}}}function renderAgentStatuses(e){const t=document.getElementById("agent-status-list");if(!t)return;const n={},s=["main","exec","coo","cfo","cmp","dev","family","tax","sec","smm"];for(const a of e){const i=a.key?.match(/^agent:([^:]+):/),r=i?i[1]:"main";n[r]||(n[r]={sessions:[],lastActivity:0,lastPreview:""}),n[r].sessions.push(a);const c=a.updatedAt?new Date(a.updatedAt).getTime():0;c>n[r].lastActivity&&(n[r].lastActivity=c,n[r].lastPreview=a.displayName||a.key||"")}for(const a of s)n[a]||(n[a]={sessions:[],lastActivity:0,lastPreview:""});const o=Object.entries(n).sort((a,i)=>i[1].lastActivity-a[1].lastActivity);t.innerHTML=o.map(([a,i])=>{const r=typeof AGENT_PERSONAS<"u"&&AGENT_PERSONAS[a],c=r?`${r.name} (${r.role})`:a.toUpperCase(),d=i.sessions.length,l=i.lastActivity?timeAgo(i.lastActivity):"No activity",m=i.lastActivity&&Date.now()-i.lastActivity<3e5,u=i.lastActivity&&Date.now()-i.lastActivity<36e5,g=m?"success":u?"warning":"idle",p=m?"Active":u?"Recent":"Idle",y=getComputedStyle(document.documentElement).getPropertyValue(`--agent-${a}`).trim()||"#888";return`
        <div class="agent-status-row" onclick="switchToAgent('${a}')" style="cursor: pointer;">
            <span class="status-dot ${g}"></span>
            <div style="flex: 1; min-width: 0;">
                <div style="display: flex; align-items: center; gap: 6px;">
                    <span style="font-weight: 600; font-size: 13px; color: ${y};">${c}</span>
        ...
        `;return}try{const n=(await gateway._request("sessions.list",{includeGlobal:!0}))?.sessions||[];let s=null;try{s=await gateway._request("status",{})}catch{}renderCostData(n,s)}catch(t){console.warn("[Costs] Failed:",t.message)}}}function renderCostData(e,t){const n=document.getElementById("cost-tracker-content");if(!n)return;const s={};let o=0,a=0,i=0;for(const l of e){const m=l.key?.match(/^agent:([^:]+):/),u=m?m[1]:"main";s[u]||(s[u]={input:0,output:0,total:0});const g=l.inputTokens||0,p=l.outputTokens||0,y=l.totalTokens||g+p;s[u].input+=g,s[u].output+=p,s[u].total+=y,o+=y,a+=g,i+=p}const r=((a*3+i*15)/1e6).toFixed(2),c=Object.entries(s).sort((l,m)=>m[1].total-l[1].total),d=c.length>0?c[0][1].total:1;n.innerHTML=`
        ...
        <div style="space-y: 4px;">
            ${c.slice(0,6).map(([l,m])=>{const u=getComputedStyle(document.documentElement).getPropertyValue(`--agent-${l}`).trim()||"#888",g=d>0?m.total/d*100:0,p=typeof getAgentLabel=="function"?getAgentLabel(l):l.toUpperCase();return`
                <div style="margin-bottom: 6px;">
                    <div style="display: flex; justify-content: space-between; font-size: 11px; margin-bottom: 2px;">
                        <span style="color: ${u}; font-weight: 600;">${p}</span>
                        <span style="color: var(--text-muted);">${formatTokens(m.total)}</span>
                    </div>
                    <div style="height: 4px; background: var(--surface-2); border-radius: 2px; overflow: hidden;">
                        <div style="height: 100%; width: ${g}%; background: ${u}; border-radius: 2px;"></div>
                    </div>
                </div>`}).join("")}
        </div>
    `}function formatTokens(e){return e>=1e6?(e/1e6).toFixed(1)+"M":e>=1e3?(e/1e3).toFixed(1)+"K":e.toString()}document.addEventListener("DOMContentLoaded",()=>{setTimeout(initCostTracker,3e3)});let analyticsInterval=null;function initAnalytics(){loadAnalyticsData(),analyticsInterval&&clearInterval(analyticsInterval),analyticsInterval=setInterval(loadAnalyticsData,6e4)}async function loadAnalyticsData(){const e=document.getElementById("analytics-content");if(e){if(!gateway||!gateway.isConnected()){e.innerHTML=`
        ...
        `;return}try{const n=(await gateway._request("sessions.list",{includeGlobal:!0}))?.sessions||[];renderAnalytics(n)}catch(t){console.warn("[Analytics] Failed:",t.message)}}}function renderAnalytics(e){const t=document.getElementById("analytics-content");if(!t)return;const n={},s={},o=Date.now();for(const c of e){const d=c.key?.match(/^agent:([^:]+):/),l=d?d[1]:"main",m=c.totalTokens||(c.inputTokens||0)+(c.outputTokens||0);if(n[l]||(n[l]=0),n[l]+=m>0?1:0,c.updatedAt){const u=new Date(c.updatedAt),g=u.toLocaleDateString("en-US",{weekday:"short"}),p=u.getTime();o-p<7*864e5&&(s[g]||(s[g]=0),s[g]++)}}const a=[...e].filter(c=>c.updatedAt).sort((c,d)=>new Date(d.updatedAt).getTime()-new Date(c.updatedAt).getTime()).slice(0,5),i=Math.max(...Object.values(n),1),r=Math.max(...Object.values(s),1);t.innerHTML=`
        <div style="margin-bottom: 12px;">
            ...
            ${Object.entries(n).sort((c,d)=>d[1]-c[1]).slice(0,6).map(([c,d])=>{const l=getComputedStyle(document.documentElement).getPropertyValue(`--agent-${c}`).trim()||"#888",m=d/i*100;return`<div style="display: flex; align-items: center; gap: 6px; margin-bottom: 3px;">
                    ...
                    <div style="flex: 1; height: 6px; background: var(--surface-2); border-radius: 3px; overflow: hidden;">
                        <div style="height: 100%; width: ${m}%; background: ${l}; border-radius: 3px;"></div>
                    </div>
...
`).filter(o=>o.trim());for(const o of s){const a=o.trim().split(/\s+/);if(a.length>=2){const i=a[0],r=a[a.length-1]||"";n.push({value:i,name:i.split("/").pop()||i,selected:r.includes("default")||r.includes("configured")})}}return n}catch{return[]}}let currentProvider="anthropic",currentModel="anthropic/claude-opus-4-5";function resolveFullModelId(e){if(!e||e.includes("/"))return e;const t={claude:"anthropic",gpt:"openai-codex",o1:"openai",o3:"openai",gemini:"google",kimi:"moonshot"};for(const[n,s]of Object.entries(t))if(e.startsWith(n))return`${s}/${e}`;return e}function syncModelDisplay(e,t){if(!e)return;const n=Date.now();if(window._lastManualModelChange&&n-window._lastManualModelChange<2e3){console.log("[Dashboard] Skipping model sync due to recent manual change");return}if(e=resolveFullModelId(e),e===currentModel&&t===currentProvider)return;console.log(`[Dashboard] Model sync: ${currentModel} \u2192 ${e} (provider: ${t||currentProvider})`),currentModel=e,!t&&e.includes("/")&&(t=e.split("/")[0]),t&&(currentProvider=t),localStorage.setItem("selected_model",e),t&&localStorage.setItem("selected_provider",t);const s=document.getElementById("current-model-display");if(s&&(s.textContent=e),t){const o=document.getElementById("current-provider-display");o&&(o.textContent=t);const a=document.getElementById("provider-select");if(a){if(!Array.from(a.options).find(c=>c.value===t)){const c=document.createElement("option");c.value=t,c.textContent=t.split("-").map(d=>d.charAt(0).toUpperCase()+d.slice(1)).join(" "),a.appendChild(c)}a.value=t}const i=document.getElementById("setting-provider");i&&(i.value=t),updateModelDropdown(t).then(()=>{selectModelInDropdowns(e)}).catch(r=>{console.warn("[Dashboard] Failed to update model dropdowns:",r),selectModelInDropdowns(e)})}else selectModelInDropdowns(e)}async function applySessionModelOverride(e){if(!e)return;let t=null;const n=availableSessions.find(o=>o.key===e),s=n?.model&&n.model!=="unknown"?n.model:null;if(s&&(t=s),!t)try{const o=await gateway?.listSessions?.({});if(o?.sessions?.length){availableSessions=o.sessions.map(r=>({key:r.key,name:getFriendlySessionName(r.key),displayName:getFriendlySessionName(r.key),updatedAt:r.updatedAt,totalTokens:r.totalTokens||(r.inputTokens||0)+(r.outputTokens||0),model:r.model||"unknown",sessionId:r.sessionId}));const a=availableSessions.find(r=>r.key===e),i=a?.model&&a.model!=="unknown"?a.model:null;i&&(t=i)}}catch(o){console.warn("[Dashboard] Failed to refresh sessions for model override:",o.message)}if(!t)try{const o=e.match(/^agent:([^:]+):/),a=o?o[1]:"main",i=await fetch(`/api/models/current?agentId=${encodeURIComponent(a)}`);if(i.ok){const r=await i.json();r?.modelId&&(t=r.modelId,console.log(`[Dashboard] Session ${e} using per-agent model from API: ${t} (agent: ${a})`))}}catch(o){console.warn("[Dashboard] Failed to fetch model config from server:",o.message)}if(t){t=resolveFullModelId(t);const o=t.includes("/")?t.split("/")[0]:currentProvider;console.log(`[Dashboard] applySessionModelOverride: Setting model to ${t} for session ${e}`),syncModelDisplay(t,o)}else console.log(`[Dashboard] applySessionModelOverride: No override for ${e}, keeping current display`)}function selectModelInDropdowns(e){const t=e.split("/").pop()||e,n=document.getElementById("model-select"),s=document.getElementById("setting-model");[n,s].forEach(o=>{if(!o)return;if(Array.from(o.options).find(r=>r.value===e))o.value=e;else{const r=document.createElement("option");r.value=e,r.textContent=t,r.selected=!0,o.appendChild(r)}})}document.addEventListener("DOMContentLoaded",async function(){try{let e=null,t=null;try{const r=(localStorage.getItem("gateway_session")||"agent:main:main").match(/^agent:([^:]+):/),c=r?r[1]:"main",l=await(await fetch(`/api/models/current?agentId=${encodeURIComponent(c)}`)).json();e=l?.modelId,t=l?.provider,console.log(`[Dashboard] Model from API: ${e} (provider: ${t}, agent: ${c})`)}catch(i){console.warn("[Dashboard] Failed to fetch current model from API:",i.message),e=localStorage.getItem("selected_model"),t=localStorage.getItem("selected_provider")}e||(e="anthropic/claude-opus-4-5"),t||(t=e.split("/")[0]),currentProvider=t,currentModel=e,console.log(`[Dashboard] Init model: ${currentModel} (provider: ${currentProvider})`),await populateProviderDropdown();const n=document.getElementById("current-provider-display"),s=document.getElementById("current-model-display"),o=document.getElementById("provider-select");n&&(n.textContent=currentProvider),s&&(s.textContent=currentModel),o&&(o.value=currentProvider);const a=document.getElementById("setting-provider");a&&(a.value=currentProvider),setInterval(async()=>{try{const r=await(await fetch("/api/models/current")).json();r?.modelId&&r?.provider&&(r.modelId!==currentModel||r.provider!==currentProvider)&&(console.log(`[Dashboard] Model changed on server: ${currentModel} \u2192 ${r.modelId}`),syncModelDisplay(r.modelId,r.provider))}catch{}},300*1e3),await updateModelDropdown(currentProvider),selectModelInDropdowns(currentModel)}catch(e){console.error("[Dashboard] Failed to initialize model display:",e)}});const defaultSettings={pickupFreq:"disabled",priorityOrder:"priority",refreshInterval:"10000",defaultPriority:"1",compactMode:!1,showLive:!0,showActivity:!0,showNotes:!0,showProducts:!0,showDocs:!0},READ_ACK_PREFIX="[[read_ack]]",unreadSessions=new Map,NOTIFICATION_DEBUG=!1;function notifLog(...e){NOTIFICATION_DEBUG&&console.log(...e)}function requestNotificationPermission(){"Notification"in window&&Notification.permission==="default"&&Notification.requestPermission().then(e=>{console.log(`[Notifications] Permission: ${e}`)})}function subscribeToAllSessions(){if(!gateway||!gateway.isConnected())return;const t=[...availableSessions].filter(n=>n.key&&n.updatedAt).sort((n,s)=>(s.updatedAt||0)-(n.updatedAt||0)).slice(0,20).map(n=>n.key);t.length>0&&(gateway.subscribeToAllSessions(t),console.log(`[Notifications] Subscribed to ${t.length} recent sessions (of ${availableSessions.length} total)`))}function handleCrossSessionNotification(e){const{sessionKey:t,content:n,images:s}=e;if(notifLog(`[Notifications] \u{1F4E5} Cross-session notification received: session=${t}, content=${(n||"").slice(0,80)}..., images=${s?.length||0}`),typeof n=="string"&&n.startsWith(READ_ACK_PREFIX)){notifLog(`[Notifications] Ignoring read-ack cross-session event for ${t}`),t&&clearUnreadForSession(t);return}if(typeof n=="string"){const i=n.trim();if(i==="NO_REPLY"||i==="NO"||i==="HEARTBEAT_OK"||i==="ANNOUNCE_SKIP"||i==="REPLY_SKIP"){notifLog(`[Notifications] Ignoring silent placeholder notification for ${t}: ${i}`);return}if(i==="[read-sync]"||i.startsWith("[[read_ack]]")||/^\[read-sync\]\s*\n*\s*\[\[read_ack\]\]$/s.test(i)){notifLog(`[Notifications] Ignoring read-sync notification for ${t}`),t&&clearUnreadForSession(t);return}}if(t&&typeof currentSessionName<"u"&&t===currentSessionName&&document.visibilityState==="visible"){notifLog(`[Notifications] Ignoring notification for active session ${t}`);return}const o=getFriendlySessionName(t),a=n.length>120?n.slice(0,120)+"\u2026":n;if(notifLog(`[Notifications] \u{1F514} Message from ${o}: ${a.slice(0,60)}`),unreadSessions.set(t,(unreadSessions.get(t)||0)+1),updateUnreadBadges(),notifLog(`[Notifications] Unread total: ${Array.from(unreadSessions.values()).reduce((i,r)=>i+r,0)}`),showNotificationToast(o,a,t),"Notification"in window&&Notification.permission==="granted")try{const i=new Notification(`${o}`,{body:a,icon:"/solobot-avatar.png",tag:`session-${t}`,silent:!1});i.onclick=()=>{window.focus(),navigateToSession(t),i.close()},setTimeout(()=>i.close(),8e3)}catch(i){console.warn("[Notifications] Browser notification failed:",i)}playNotificationSound()}function navigateToSession(e){typeof showPage=="function"&&showPage("chat");const t=e.match(/^agent:([^:]+):/);t&&typeof setActiveSidebarAgent=="function"&&setActiveSidebarAgent(t[1]),typeof switchToSessionKey=="function"&&switchToSessionKey(e),unreadSessions.delete(e),updateUnreadBadges()}function showNotificationToast(e,t,n){let s=document.getElementById("notification-toast-container");s||(s=document.createElement("div"),s.id="notification-toast-container",s.style.cssText="position: fixed; top: 12px; right: 12px; z-index: 10000; display: flex; flex-direction: column; gap: 8px; max-width: 360px; pointer-events: none;",document.body.appendChild(s));const o=n?.match(/^agent:([^:]+):/),a=o?o[1]:"main",r={main:"#BC2026",dev:"#6366F1",exec:"#F59E0B",coo:"#10B981",cfo:"#EAB308",cmp:"#EC4899",family:"#14B8A6",tax:"#78716C",sec:"#3B82F6",smm:"#8B5CF6"}[a]||"#BC2026",c=document.createElement("div");c.className="notification-toast",c.style.cssText=`
    ...
    `,c.addEventListener("click",l=>{if(l.target.classList?.contains("toast-close")){dismissToast(c);return}navigateToSession(n),dismissToast(c)}),s.appendChild(c),notifLog(`[Notifications] Toast rendered for ${e} (session=${n})`),requestAnimationFrame(()=>{c.style.opacity="1",c.style.transform="translateX(0)"});const d=setTimeout(()=>dismissToast(c),12e3);for(c._dismissTimer=d;s.children.length>4;)dismissToast(s.firstChild)}function dismissToast(e){!e||e._dismissed||(e._dismissed=!0,clearTimeout(e._dismissTimer),e.style.opacity="0",e.style.transform="translateX(100%)",setTimeout(()=>e.remove(),300))}function toggleNotificationPanel(){if(unreadSessions.size===0){const n=document.getElementById("notification-bell");n&&(n.style.animation="none",n.offsetHeight,n.style.animation="bellPulse 0.3s ease-in-out");return}let e=null,t=0;for(const[n,s]of unreadSessions)s>t&&(t=s,e=n);e&&navigateToSession(e)}function playNotificationSound(){try{const e=new(window.AudioContext||window.webkitAudioContext),t=e.createOscillator(),n=e.createGain();t.connect(n),n.connect(e.destination),t.frequency.setValueAtTime(880,e.currentTime),t.frequency.setValueAtTime(1047,e.currentTime+.1),n.gain.setValueAtTime(.08,e.currentTime),n.gain.exponentialRampToValueAtTime(.001,e.currentTime+.3),t.start(e.currentTime),t.stop(e.currentTime+.3)}catch{}}function updateUnreadBadges(){document.querySelectorAll(".session-option, [data-session-key]").forEach(s=>{const o=s.dataset?.sessionKey||s.getAttribute("data-session-key");if(!o)return;let a=s.querySelector(".unread-badge");const i=unreadSessions.get(o)||0;i>0?(a||(a=document.createElement("span"),a.className="unread-badge",s.appendChild(a)),a.textContent=i>99?"99+":i,a.style.cssText="background: var(--brand-red, #BC2026); color: white; border-radius: 50%; min-width: 18px; height: 18px; font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; justify-content: center; padding: 0 4px; margin-left: 6px;"):a&&a.remove()}),document.querySelectorAll(".sidebar-agent[data-agent]").forEach(s=>{const o=s.getAttribute("data-agent");if(!o)return;let a=0;for(const[r,c]of unreadSessions)(r.startsWith(`agent:${o}:`)||o==="main"&&r==="main")&&(a+=c);let i=s.querySelector(".agent-unread-dot");a>0?(i||(i=document.createElement("span"),i.className="agent-unread-dot",i.style.cssText="position: absolute; top: 2px; right: 2px; background: var(--brand-red, #BC2026); color: white; border-radius: 50%; min-width: 16px; height: 16px; font-size: 10px; font-weight: 700; display: flex; align-items: center; justify-content: center; padding: 0 3px; pointer-events: none;",s.style.position="relative",s.appendChild(i)),i.textContent=a>99?"99+":a):i&&i.remove()});const e=Array.from(unreadSessions.values()).reduce((s,o)=>s+o,0),t=document.getElementById("notification-bell-badge");t&&(e>0?(t.textContent=e>99?"99+":e,t.style.display="flex"):t.style.display="none");const n="SoLoVision Dashboard";document.title=e>0?`(${e}) ${n}`:n}async function sendReadAck(e){try{gateway&&gateway.isConnected()&&await gateway.injectChat(e,READ_ACK_PREFIX,"read-sync")}catch(t){console.warn("[Notifications] Failed to send read ack:",t.message)}}function clearUnreadForSession(e){unreadSessions.has(e)&&(unreadSessions.delete(e),updateUnreadBadges(),sendReadAck(e))}function connectToGateway(){const e=document.getElementById("gateway-host")?.value||GATEWAY_CONFIG.host,t=parseInt(document.getElementById("gateway-port")?.value)||GATEWAY_CONFIG.port,n=document.getElementById("gateway-token")?.value||GATEWAY_CONFIG.token,s=document.getElementById("gateway-session")?.value||GATEWAY_CONFIG.sessionKey||"agent:main:main",o=s==="main"?"agent:main:main":s;if(!e){showToast("Please enter a gateway host in Settings","warning");return}if(gateway&&gateway.isConnected()&&gateway.sessionKey===o){console.log("[Dashboard] Already connected with correct session, skipping reconnect");return}GATEWAY_CONFIG.host=e,GATEWAY_CONFIG.port=t,GATEWAY_CONFIG.token=n,GATEWAY_CONFIG.sessionKey=o,saveGatewaySettings(e,t,n,o),updateConnectionUI("connecting","Connecting..."),gateway||initGateway(),gateway.sessionKey=o,gateway.connect(e,t,n)}function disconnectFromGateway(){gateway&&gateway.disconnect(),updateConnectionUI("disconnected","Disconnected")}window.requestGatewayRestart=async function(){if(!gateway||!gateway.isConnected()){showToast("Not connected to gateway","warning");return}showToast("Restarting gateway...","info");try{await gateway.restartGateway("manual restart from dashboard"),showToast("Gateway restart initiated. Reconnecting...","success")}catch(e){console.error("[Dashboard] Gateway restart failed:",e),showToast("Restart failed: "+e.message,"error")}};function updateConnectionUI(e,t){const n=document.getElementById("gateway-status"),s=document.getElementById("gateway-status-dot"),o=document.getElementById("settings-gateway-status"),a=document.getElementById("settings-gateway-dot"),i=document.getElementById("gateway-connect-btn"),r=document.getElementById("gateway-disconnect-btn"),c=t||e;n&&(n.textContent=c),o&&(o.textContent=c);const l=(()=>{switch(e){case"connected":return"success";case"connecting":return"warning pulse";case"error":return"error";default:return"idle"}})();s&&(s.className=`status-dot ${l}`),a&&(a.className=`status-dot ${l}`),i&&r&&(e==="connected"?(i.classList.add("hidden"),r.classList.remove("hidden")):(i.classList.remove("hidden"),r.classList.add("hidden"))),renderChat(),renderChatPage()}function handleChatEvent(e){const{state:t,content:n,images:s,role:o,errorMessage:a,model:i,provider:r,stopReason:c,sessionKey:d,runId:l}=e,m=currentSessionName?.toLowerCase(),u=d?.toLowerCase();if(!(u&&m&&u!==m)){if(n&&n.startsWith(READ_ACK_PREFIX)){d&&clearUnreadForSession(d);return}if(d&&d.startsWith("health-check-")){const g=pendingHealthChecks.get(d);g&&(t==="final"?(g.resolve({success:!0,content:n,model:i,provider:r}),pendingHealthChecks.delete(d)):t==="error"&&(g.reject(new Error(a||"Gateway error")),pendingHealthChecks.delete(d)));return}if(i&&(window._lastResponseModel=i,window._lastResponseProvider=r,syncModelDisplay(i,r)),o==="user"&&t==="final"&&n){const g=(currentSessionName||GATEWAY_CONFIG?.sessionKey||"").toLowerCase(),p=d?.toLowerCase();if(p&&g&&p!==g){notifLog(`[Notifications] Ignoring user message for session ${p} (current: ${g})`);return}state.chat.messages.some(f=>f.from==="user"&&f.text?.trim()===n.trim()&&Date.now()-f.time<5e3)||addLocalChatMessage(n,"user");return}switch(t){case"start":case"thinking":isProcessing=!0,streamingText="",renderChat(),renderChatPage();break;case"delta":streamingText=n,_streamingSessionKey=d||currentSessionName||"",isProcessing=!0,renderChat(),renderChatPage();break;case"final":const g=streamingText||n;if(g&&/^\s*\[read-sync\]\s*(\n\s*\[\[read_ack\]\])?\s*$/s.test(g)){streamingText="",isProcessing=!1,lastProcessingEndTime=Date.now();break}if(g&&o!=="user"){const p=g.trim();if(!state.chat.messages.some(f=>l&&f.runId===l||f.from==="solobot"&&f.text?.trim()===p&&Date.now()-f.time<1e4)){const f=addLocalChatMessage(g,"solobot",s,window._lastResponseModel);f&&l&&(f.runId=l)}}streamingText="",isProcessing=!1,lastProcessingEndTime=Date.now(),setTimeout(_doHistoryRefresh,2e3),renderChat(),renderChatPage();break;case"error":addLocalChatMessage(`Error: ${a||"Unknown error"}`,"system"),streamingText="",isProcessing=!1,lastProcessingEndTime=Date.now(),renderChat(),renderChatPage();break}}}function loadHistoryMessages(e){const t=(currentSessionName||GATEWAY_CONFIG?.sessionKey||"").toLowerCase(),n=state.chat.messages.filter(d=>!(!d.id?.startsWith("m")||d._sessionKey&&d._sessionKey.toLowerCase()!==t)),s=[],o=[],a=d=>{if(!d)return{text:"",images:[]};let l="",m=[];if(Array.isArray(d.content))for(const u of d.content)if(u.type==="text")l+=u.text||"";else if(u.type==="input_text")l+=u.text||u.input_text||"";else if(u.type==="image")if(u.content&&u.mimeType)m.push(`data:${u.mimeType};base64,${u.content}`);else if(u.source?.data){const g=u.source.media_type||"image/jpeg";m.push(`data:${g};base64,${u.source.data}`)}else u.data&&m.push(`data:image/jpeg;base64,${u.data}`);else u.type==="image_url"&&u.image_url?.url&&m.push(u.image_url.url);else typeof d.content=="string"&&(l=d.content);if(Array.isArray(d.attachments))for(const u of d.attachments)u.type==="image"&&u.content&&u.mimeType&&m.push(`data:${u.mimeType};base64,${u.content}`);return!l&&typeof d.text=="string"&&(l=d.text),{text:(l||"").trim(),images:m}};e.forEach(d=>{if(d.role==="toolResult"||d.role==="tool"||d.model==="gateway-injected"||d.provider==="openclaw")return;let l=a(d);!l.text&&!l.images.length&&d.message&&(l=a(d.message));const m={id:d.id||"m"+Date.now()+Math.random(),from:d.role==="user"?"user":"solobot",text:l.text,image:l.images[0]||null,images:l.images,time:d.timestamp||Date.now()};isSystemMessage(l.text,m.from)?o.push(m):s.push(m)});const i=new Set(s.map(d=>d.id)),r=new Set(s.map(d=>d.text)),c=n.filter(d=>!i.has(d.id)&&!r.has(d.text));state.chat.messages=[...s,...c],console.log(`[Dashboard] Set ${state.chat.messages.length} chat messages (${s.length} from history, ${c.length} local)`),state.chat.messages.sort((d,l)=>d.time-l.time),state.chat.messages.length>GATEWAY_CONFIG.maxMessages&&(state.chat.messages=state.chat.messages.slice(-GATEWAY_CONFIG.maxMessages)),state.system.messages=[...state.system.messages,...o],state.system.messages.sort((d,l)=>d.time-l.time),state.system.messages.length>GATEWAY_CONFIG.maxMessages&&(state.system.messages=state.system.messages.slice(-GATEWAY_CONFIG.maxMessages)),persistSystemMessages(),persistChatMessages(),renderChat(),renderChatPage(),renderSystemPage()}let _historyRefreshFn=null,_historyVisibilityFn=null,_historyRefreshInFlight=!1,_lastHistoryLoadTime=0;const HISTORY_MIN_INTERVAL=8e3;function _doHistoryRefresh(){if(!gateway||!gateway.isConnected()||isProcessing||Date.now()-lastProcessingEndTime<1500||_historyRefreshInFlight||Date.now()-_lastHistoryLoadTime<HISTORY_MIN_INTERVAL)return;_historyRefreshInFlight=!0,_lastHistoryLoadTime=Date.now();const e=sessionVersion,t=GATEWAY_CONFIG?.sessionKey||"unknown";notifLog(`[Notifications] _doHistoryRefresh: session=${t}, version=${e}`),gateway.loadHistory().then(n=>{if(_historyRefreshInFlight=!1,e!==sessionVersion){notifLog(`[Notifications] _doHistoryRefresh: Skipped (version mismatch ${e} vs ${sessionVersion})`);return}n?.messages?(notifLog(`[Notifications] _doHistoryRefresh: Got ${n.messages.length} messages for session=${t}`),mergeHistoryMessages(n.messages)):notifLog("[Notifications] _doHistoryRefresh: No messages returned")}).catch(n=>{_historyRefreshInFlight=!1,notifLog(`[Notifications] _doHistoryRefresh: Error - ${n.message}`)})}function startHistoryPolling(){stopHistoryPolling(),historyPollInterval=setInterval(_doHistoryRefresh,3e4),_historyRefreshFn||(_historyRefreshFn=_doHistoryRefresh,_historyVisibilityFn=()=>{document.visibilityState==="visible"&&_doHistoryRefresh()},window.addEventListener("focus",_historyRefreshFn),document.addEventListener("visibilitychange",_historyVisibilityFn))}function stopHistoryPolling(){historyPollInterval&&(clearInterval(historyPollInterval),historyPollInterval=null),_historyRefreshFn&&(window.removeEventListener("focus",_historyRefreshFn),document.removeEventListener("visibilitychange",_historyVisibilityFn),_historyRefreshFn=null,_historyVisibilityFn=null)}function mergeHistoryMessages(e){const t=(currentSessionName||GATEWAY_CONFIG?.sessionKey||"").toLowerCase();if(!t){notifLog("[Notifications] mergeHistoryMessages: No active session, skipping merge");return}const n=new Set(state.chat.messages.map(l=>l.id)),s=new Set(state.system.messages.map(l=>l.id)),o=new Set(state.chat.messages.map(l=>(l.text||"").trim())),a=new Set(state.system.messages.map(l=>(l.text||"").trim())),i=new Set(state.chat.messages.filter(l=>l.runId).map(l=>l.runId));let r=0,c=0;const d=l=>{if(!l)return"";let m="";if(Array.isArray(l.content))for(const u of l.content)u.type==="text"&&(m+=u.text||""),u.type==="input_text"&&(m+=u.text||u.input_text||"");else typeof l.content=="string"&&(m=l.content);return!m&&typeof l.text=="string"&&(m=l.text),(m||"").trim()};for(const l of e){const m=l.id||"m"+l.timestamp;if(!(n.has(m)||s.has(m))&&!(l.role==="toolResult"||l.role==="tool")&&!(l.model==="gateway-injected"||l.provider==="openclaw")){let u=d(l);if(!u&&l.message&&(u=d(l.message)),u){const g=isSystemMessage(u,l.role==="user"?"user":"solobot");if(l.runId&&i.has(l.runId)||g&&a.has(u)||!g&&o.has(u)||l.role!=="user"&&state.chat.messages.some(f=>f.from==="solobot"&&Date.now()-f.time<5e3)&&!n.has(m)&&state.chat.messages.some(v=>v.from==="solobot"&&Date.now()-v.time<5e3&&v.text?.trim()===u))continue;const p={id:m,from:l.role==="user"?"user":"solobot",text:u,time:l.timestamp||Date.now()};g?(state.system.messages.push(p),a.add(u),c++):(state.chat.messages.push(p),n.add(m),o.add(u),r++)}}}if(r>0||c>0){notifLog(`[Notifications] mergeHistoryMessages: Merged ${r} chat, ${c} system messages for session ${t}`),state.chat.messages.sort((u,g)=>u.time-g.time),state.chat.messages.length>GATEWAY_CONFIG.maxMessages&&(state.chat.messages=state.chat.messages.slice(-GATEWAY_CONFIG.maxMessages)),state.system.messages.sort((u,g)=>u.time-g.time),state.system.messages.length>GATEWAY_CONFIG.maxMessages&&(state.system.messages=state.system.messages.slice(-GATEWAY_CONFIG.maxMessages)),persistSystemMessages(),persistChatMessages();const l=window.getSelection();if(!(l&&l.toString().trim().length>0))renderChat(),renderChatPage(),renderSystemPage();else if(console.log("[Dashboard] Deferring render \u2014 text is selected"),!window._pendingRender){window._pendingRender=!0;const u=()=>{const g=window.getSelection();!g||g.toString().trim().length===0?(window._pendingRender=!1,renderChat(),renderChatPage(),renderSystemPage()):requestAnimationFrame(u)};requestAnimationFrame(u)}}}let securityInterval=null;function initSecurityPage(){loadSecurityData(),securityInterval&&clearInterval(securityInterval),securityInterval=setInterval(loadSecurityData,3e4)}async function loadSecurityData(){if(!gateway||!gateway.isConnected()){renderSecurityDisconnected();return}try{const e=await gateway._request("exec.approvals.list",{});renderExecApprovals(e?.items||e||[])}catch{renderExecApprovals([])}try{const e=await gateway._request("audit.events",{limit:50});renderConnectionLog(e?.events||e||[])}catch{renderConnectionLog([])}try{const e=await gateway._request("devices.list",{});renderDevices(e?.devices||e||[])}catch{renderDevices([])}}function renderSecurityDisconnected(){const e=document.getElementById("exec-approvals");e&&(e.innerHTML='<div class="empty-state">Connect to gateway</div>');const t=document.getElementById("connection-log");t&&(t.innerHTML="")}function renderExecApprovals(e){const t=document.getElementById("exec-approvals");if(t){if(e.length===0){t.innerHTML='<div style="color: var(--text-muted); font-size: 12px; text-align: center; padding: 16px;">No pending approvals</div>';return}t.innerHTML=e.map(n=>{const s=n.status||"pending",o=s==="approved"?"success":s==="denied"?"error":"warning",a=n.createdAt?new Date(n.createdAt).toLocaleString():"";return`
        ...
        </div>`).join("")}}const SESSION_DEBUG=!1;window.sessLog=function(...e){SESSION_DEBUG&&console.log(...e)};function sessLog(...e){SESSION_DEBUG&&console.log(...e)}const AGENT_PERSONAS={main:{name:"Halo",role:"PA"},exec:{name:"Elon",role:"CoS"},cto:{name:"Orion",role:"CTO"},coo:{name:"Atlas",role:"COO"},cfo:{name:"Sterling",role:"CFO"},cmp:{name:"Vector",role:"CMP"},dev:{name:"Dev",role:"ENG"},forge:{name:"Forge",role:"DEVOPS"},quill:{name:"Quill",role:"FE/UI"},chip:{name:"Chip",role:"SWE"},snip:{name:"Snip",role:"YT"},sec:{name:"Knox",role:"SEC"},smm:{name:"Nova",role:"SMM"},family:{name:"Haven",role:"FAM"},tax:{name:"Ledger",role:"TAX"},docs:{name:"Canon",role:"DOC"}};function normalizeDashboardSessionKey(e){return!e||e==="main"?"agent:main:main":e}function getFriendlySessionName(e){if(!e)return"Halo (PA)";const t=e.match(/^agent:([^:]+):(.+)$/);if(t){const n=t[1],s=t[2],o=AGENT_PERSONAS[n],a=o?o.name:n.toUpperCase();return s==="main"?a:`${a} (${s})`}return e}let currentSessionName;window.currentSessionName=currentSessionName;function initCurrentSessionName(){const e=localStorage.getItem("gateway_session"),t=typeof GATEWAY_CONFIG<"u"&&GATEWAY_CONFIG?.sessionKey?GATEWAY_CONFIG.sessionKey:null;currentSessionName=normalizeDashboardSessionKey(e||t||"agent:main:main"),window.currentSessionName=currentSessionName,console.log("[initCurrentSessionName] localStorage:",e,"Final:",currentSessionName)}initCurrentSessionName(),window.toggleSessionMenu=function(){const e=document.getElementById("session-menu");e&&e.classList.toggle("hidden")},window.renameSession=async function(){toggleSessionMenu();const e=prompt("Enter new session name:",currentSessionName);if(!(!e||e===currentSessionName))try{const t=await fetch("/api/session/rename",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({oldName:currentSessionName,newName:e})});if(t.ok){currentSessionName=e,window.currentSessionName=currentSessionName;const n=document.getElementById("current-session-name");n&&(n.textContent=e),showToast(`Session renamed to "${e}"`,"success")}else{const n=await t.json();showToast(`Failed to rename: ${n.error||"Unknown error"}`,"error")}}catch(t){console.error("[Dashboard] Failed to rename session:",t),showToast("Failed to rename session","error")}},window.showSessionSwitcher=function(){toggleSessionMenu(),showToast("Session switcher coming soon","info")},window.toggleChatPageSessionMenu=function(){const e=document.getElementById("chat-page-session-menu");e&&e.classList.toggle("hidden")},document.addEventListener("click",function(e){const t=document.getElementById("chat-page-session-menu"),n=e.target.closest('[onclick*="toggleChatPageSessionMenu"]');t&&!t.classList.contains("hidden")&&!t.contains(e.target)&&!n&&t.classList.add("hidden")});let availableSessions=[],currentAgentId="main";window.currentAgentId=currentAgentId;let _switchInFlight=!1,_sessionSwitchQueue=[];function getAgentIdFromSession(e){const t=e?.match(/^agent:([^:]+):/);return t?t[1]:"main"}function filterSessionsForAgent(e,t){return e.filter(n=>!!(getAgentIdFromSession(n.key)===t||n.key?.startsWith("agent:main:subagent:")&&(n.displayName||n.name||"").toLowerCase().startsWith(t.toLowerCase()+"-")))}function checkUrlSessionParam(){return new URLSearchParams(window.location.search).get("session")}function handleSubagentSessionAgent(){if(!currentSessionName?.startsWith("agent:main:subagent:"))return;const e=availableSessions.find(s=>s.key===currentSessionName);if(!e){sessLog(`[Dashboard] Subagent session not found in available sessions: ${currentSessionName}`);return}const t=e.displayName||e.name||"";sessLog(`[Dashboard] Subagent session label: ${t}`);const n=t.match(/^([a-z]+)-/i);if(n){const s=n[1].toLowerCase();sessLog(`[Dashboard] Determined agent from label: ${s}`),currentAgentId=s,window.currentAgentId=s,setActiveSidebarAgent(s);const o=document.getElementById("chat-page-agent-name");o&&(o.textContent=getAgentLabel(s))}}let _fetchSessionsInFlight=!1,_fetchSessionsQueued=!1;async function fetchSessions(){if(_fetchSessionsInFlight)return _fetchSessionsQueued=!0,availableSessions;_fetchSessionsInFlight=!0;try{const e=availableSessions.filter(r=>r.sessionId===null);if(gateway&&gateway.isConnected())try{const d=((await gateway.listSessions({}))?.sessions||[]).map(u=>{const g=getFriendlySessionName(u.key);return{key:u.key,name:g,displayName:g,updatedAt:u.updatedAt,totalTokens:u.totalTokens||(u.inputTokens||0)+(u.outputTokens||0),model:u.model||"unknown",sessionId:u.sessionId}}),l=new Set(d.map(u=>u.key)),m=e.filter(u=>!l.has(u.key));if(availableSessions=[...d,...m],sessLog(`[Dashboard] Fetched ${d.length} from gateway + ${m.length} local = ${availableSessions.length} total`),handleSubagentSessionAgent(),populateSessionDropdown(),typeof updateSidebarAgentsFromSessions=="function")try{updateSidebarAgentsFromSessions(availableSessions)}catch(u){console.warn("[SidebarAgents] update failed:",u.message)}return subscribeToAllSessions(),availableSessions}catch(r){console.warn("[Dashboard] Gateway sessions.list failed, falling back to server:",r.message)}const t=await fetch("/api/sessions");if(!t.ok)throw new Error(`HTTP ${t.status}`);const o=((await t.json()).sessions||[]).map(r=>{const c=getFriendlySessionName(r.key);return{key:r.key,name:c,displayName:r.displayName||c,updatedAt:r.updatedAt,totalTokens:r.totalTokens||(r.inputTokens||0)+(r.outputTokens||0),model:r.model||"unknown",sessionId:r.sessionId}}),a=new Set(o.map(r=>r.key)),i=e.filter(r=>!a.has(r.key));if(availableSessions=[...o,...i],sessLog(`[Dashboard] Fetched ${o.length} from server + ${i.length} local = ${availableSessions.length} total`),handleSubagentSessionAgent(),populateSessionDropdown(),typeof updateSidebarAgentsFromSessions=="function")try{updateSidebarAgentsFromSessions(availableSessions)}catch(r){console.warn("[SidebarAgents] update failed:",r.message)}return availableSessions}catch(e){return console.error("[Dashboard] Failed to fetch sessions:",e),[]}finally{_fetchSessionsInFlight=!1,_fetchSessionsQueued&&(_fetchSessionsQueued=!1,setTimeout(fetchSessions,100))}}function populateSessionDropdown(){const e=document.getElementById("chat-page-session-menu");if(!e)return;const t=filterSessionsForAgent(availableSessions,currentAgentId);sessLog(`[Dashboard] populateSessionDropdown: agent=${currentAgentId}, total=${availableSessions.length}, filtered=${t.length}`),sessLog("[Dashboard] Available sessions:",availableSessions.map(o=>o.key));let n="";const s=getAgentLabel(currentAgentId);if(n+=`<div style="padding: 8px 12px; font-size: 11px; text-transform: uppercase; color: var(--text-muted); border-bottom: 1px solid var(--border-default); display: flex; justify-content: space-between; align-items: center;">
...
This will permanently delete all messages in this session.`))try{if(gateway&&gateway.isConnected()){const n=await gateway.request("sessions.delete",{sessionKey:e});n&&n.ok?(showToast(`Session "${t}" deleted`,"success"),availableSessions=availableSessions.filter(s=>s.key!==e),populateSessionDropdown()):showToast(`Failed to delete: ${n?.error||"Unknown error"}`,"error")}else showToast("Not connected to gateway","error")}catch(n){console.error("[Dashboard] Failed to delete session:",n),showToast("Failed to delete session: "+n.message,"error")}},window.switchToSessionKey=window.switchToSession=async function(e){if(e=normalizeDashboardSessionKey(e),_sessionSwitchQueue.push({sessionKey:e,timestamp:Date.now()}),!_switchInFlight)for(;_sessionSwitchQueue.length>0;){const{sessionKey:t}=_sessionSwitchQueue.shift();if(t===currentSessionName){populateSessionDropdown();continue}await executeSessionSwitch(t)}};async function executeSessionSwitch(e){_switchInFlight=!0;try{toggleChatPageSessionMenu(),clearUnreadForSession(e),showToast(`Switching to ${getFriendlySessionName(e)}...`,"info"),streamingText="",_streamingSessionKey="",isProcessing=!1,state.chat.messages=[],renderChat(),renderChatPage(),await saveCurrentChat(),cacheSessionMessages(currentSessionName||GATEWAY_CONFIG.sessionKey,state.chat.messages),sessionVersion++,sessLog(`[Dashboard] Session version now ${sessionVersion}`);const t=currentSessionName;currentSessionName=e,window.currentSessionName=currentSessionName,GATEWAY_CONFIG.sessionKey=e,localStorage.setItem("gateway_session",e);const n=document.getElementById("gateway-session");n&&(n.value=e);const s=e.match(/^agent:([^:]+):/);s&&(currentAgentId=s[1],window.currentAgentId=s[1],typeof forceSyncActiveAgent=="function"&&forceSyncActiveAgent(s[1])),await clearChatHistory(!0,!0);const o=getCachedSessionMessages(e);o&&o.length>0&&(state.chat.messages=o.slice(),typeof renderChatPage=="function"&&renderChatPage(),sessLog(`[Dashboard] Restored ${o.length} cached messages for ${e}`)),await new Promise(c=>setTimeout(c,50)),gateway&&gateway.isConnected()?gateway.setSessionKey(e):gateway&&(sessLog("[Dashboard] Gateway not connected, initiating connect..."),connectToGateway());const a=loadSessionHistory(e),i=applySessionModelOverride(e).catch(()=>{}),r=document.getElementById("chat-page-session-name");r&&(r.textContent=e,r.title=e),populateSessionDropdown(),await a,await i,s?(setActiveSidebarAgent(s[1]),saveLastAgentSession(s[1],e)):setActiveSidebarAgent(null),showToast(`Switched to ${getFriendlySessionName(e)}`,"success")}catch(t){console.error("[Dashboard] Failed to switch session:",t),showToast("Failed to switch session","error")}finally{_switchInFlight=!1}}window.goToSession=async function(e){if(e=normalizeDashboardSessionKey(e),!e){showToast("No session key provided","warning");return}if(sessLog(`[Dashboard] goToSession called with: ${e}`),!gateway||!gateway.isConnected()){showToast("Connecting to gateway...","info"),GATEWAY_CONFIG.sessionKey=e,currentSessionName=e,window.currentSessionName=currentSessionName,localStorage.setItem("gateway_session",e);const t=document.getElementById("gateway-session");t&&(t.value=e),GATEWAY_CONFIG.host?connectToGateway():showToast("Please configure gateway settings first","warning");return}showPage("chat"),await switchToSession(e)},window.getSessionUrl=function(e){return`${window.location.origin+window.location.pathname}?session=${encodeURIComponent(e)}`};async function saveCurrentChat(){try{const t=await(await fetch("/api/state")).json();t.archivedChats||(t.archivedChats={}),t.archivedChats[currentSessionName]={savedAt:Date.now(),messages:chatHistory.slice(-100)},await fetch("/api/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}catch{}}async function loadSessionHistory(e){const t=sessionVersion;async function n(){if(!gateway||!gateway.isConnected())return!1;try{const o=await gateway.loadHistory();if(t!==sessionVersion)return sessLog(`[Dashboard] Ignoring stale history load for ${e}`),!0;if(o?.messages&&o.messages.length>0)return state.chat?.messages?.length>0?mergeHistoryMessages(o.messages):loadHistoryMessages(o.messages),sessLog(`[Dashboard] Loaded ${o.messages.length} messages from gateway for ${e}`),!0}catch(o){console.warn("[Dashboard] Gateway history failed:",o.message)}return!1}if(await n()||gateway&&!gateway.isConnected()&&(sessLog(`[Dashboard] Gateway disconnected during switch to ${e}, waiting for reconnect...`),await new Promise(o=>setTimeout(o,2e3)),t!==sessionVersion||await n()))return;const s=getCachedSessionMessages(e);if(s&&s.length>0){state.chat.messages=s.slice(),renderChat(),renderChatPage(),sessLog(`[Dashboard] Loaded ${s.length} cached messages for ${e}`);return}sessLog(`[Dashboard] No history available for ${e} \u2014 rendering empty`),renderChat(),renderChatPage()}async function loadArchivedChat(e){chatHistory=[],renderChat(),renderChatPage()}function initGateway(){gateway=new GatewayClient({sessionKey:GATEWAY_CONFIG.sessionKey,onConnected:(e,t)=>{sessLog(`[Dashboard] Connected to ${e}, session: ${t}`),updateConnectionUI("connected",e);const n=normalizeDashboardSessionKey(GATEWAY_CONFIG.sessionKey||currentSessionName||t);t!==n&&(sessLog(`[Dashboard] Reconnect mismatch: gateway=${t}, intended=${n}. Re-syncing gateway.`),gateway&&gateway.setSessionKey(n)),GATEWAY_CONFIG.sessionKey=n,currentSessionName=n,window.currentSessionName=currentSessionName,fetchModelsFromGateway().then(()=>applySessionModelOverride(n));const s=document.getElementById("current-session-name");s&&(s.textContent=n,s.title=n);const o=document.getElementById("chat-page-session-name");o&&(o.textContent=n,o.title=n);const a=n.match(/^agent:([^:]+):/);a&&saveLastAgentSession(a[1],n),checkRestartToast(),_historyRefreshInFlight=!0,_lastHistoryLoadTime=Date.now();const i=sessionVersion;gateway.loadHistory().then(r=>{if(_historyRefreshInFlight=!1,i!==sessionVersion){sessLog(`[Dashboard] Ignoring stale history (version ${i} != ${sessionVersion})`);return}if(r?.messages){if(!state.chat?.messages?.length)try{const c=chatStorageKey(),d=localStorage.getItem(c);if(d){const l=JSON.parse(d);Array.isArray(l)&&l.length>0&&(state.chat.messages=l,sessLog(`[Dashboard] Restored ${l.length} messages from localStorage before merge`))}}catch{}mergeHistoryMessages(r.messages)}}).catch(()=>{_historyRefreshInFlight=!1}),startHistoryPolling(),fetchSessions()},onDisconnected:e=>{updateConnectionUI("disconnected",e),isProcessing=!1,streamingText="",stopHistoryPolling()},onChatEvent:e=>{handleChatEvent(e)},onToolEvent:e=>{e.phase==="start"&&e.summary&&addTerminalLog(e.summary,"info",e.timestamp)},onCrossSessionMessage:e=>{handleCrossSessionNotification(e)},onError:e=>{console.error(`[Dashboard] Gateway error: ${e}`),updateConnectionUI("error",e)}})}let newSessionModalResolve=null;window.openNewSessionModal=function(e){return new Promise(t=>{newSessionModalResolve=t;const n=document.getElementById("new-session-modal"),s=document.getElementById("new-session-name-input");n&&s?(s.value=e||"",n.classList.add("visible"),setTimeout(()=>{s.focus(),s.select()},50)):t(null)})},window.closeNewSessionModal=function(e){const t=document.getElementById("new-session-modal");t&&t.classList.remove("visible"),newSessionModalResolve&&(newSessionModalResolve(e),newSessionModalResolve=null)},window.submitNewSessionModal=function(){const e=document.getElementById("new-session-name-input"),t=e?e.value:null;closeNewSessionModal(t)},document.addEventListener("keydown",function(e){const t=document.getElementById("new-session-modal");t&&t.classList.contains("visible")&&(e.key==="Enter"?(e.preventDefault(),submitNewSessionModal()):e.key==="Escape"&&(e.preventDefault(),closeNewSessionModal(null)))}),window.startNewAgentSession=async function(e){toggleChatPageSessionMenu();const t=new Date,n=String(t.getMonth()+1).padStart(2,"0"),s=String(t.getDate()).padStart(2,"0"),o=t.getFullYear();let a=t.getHours();const i=a>=12?"PM":"AM";a=a%12,a=a||12;const r=String(t.getMinutes()).padStart(2,"0"),c=String(t.getSeconds()).padStart(2,"0"),d=`${n}/${s}/${o} ${String(a).padStart(2,"0")}:${r}:${c} ${i}`,l=getAgentLabel(e),m=await openNewSessionModal(d);if(!m||!m.trim())return;const u=`${e.toLowerCase()}-${m.trim()}`,g=`agent:${e}:${u}`;if(availableSessions.some(w=>w.key===g)){showToast(`Session "${m.trim()}" already exists. Switching to it.`,"info"),await switchToSession(g);return}showToast(`Creating new ${l} session "${m.trim()}"...`,"info"),sessionVersion++,sessLog(`[Dashboard] Session version now ${sessionVersion} (new agent session)`),state.chat.messages=[],state.system.messages=[],chatPageNewMessageCount=0,chatPageUserScrolled=!1,localStorage.removeItem(chatStorageKey()),currentAgentId=e,window.currentAgentId=e,renderChat(),renderChatPage(),currentSessionName=g,window.currentSessionName=currentSessionName,GATEWAY_CONFIG.sessionKey=g,localStorage.setItem("gateway_session",g),typeof saveGatewaySettings=="function"?saveGatewaySettings(GATEWAY_CONFIG.host,GATEWAY_CONFIG.port,GATEWAY_CONFIG.token,g):typeof state<"u"&&state.gatewayConfig&&(state.gatewayConfig.sessionKey=g,typeof saveState=="function"&&saveState("Updated session for reload"));const p=document.getElementById("gateway-session");p&&(p.value=g);const y=m.trim(),f=document.getElementById("chat-page-session-name");f&&(f.textContent=y),streamingText="",isProcessing=!1,gateway&&gateway.isConnected()?gateway.setSessionKey(g):gateway&&connectToGateway();const v={key:g,name:u,displayName:y,updatedAt:Date.now(),totalTokens:0,model:currentModel||"unknown",sessionId:null};availableSessions.unshift(v),await fetchSessions(),availableSessions.some(w=>w.key===g)||availableSessions.unshift(v),populateSessionDropdown(),setActiveSidebarAgent(e),renderChat(),renderChatPage(),renderSystemPage(),showToast(`New ${l} session "${y}" created`,"success")},window.startNewSession=async function(){await startNewAgentSession(currentAgentId)};let sessionSearchQuery="";function filterSessionsBySearch(e,t){if(!t)return e;const n=t.toLowerCase();return e.filter(s=>{const o=(s.displayName||s.name||s.key||"").toLowerCase(),a=(s.model||"").toLowerCase();return o.includes(n)||a.includes(n)})}const originalPopulateSessionDropdown=window.populateSessionDropdown;typeof originalPopulateSessionDropdown=="function"&&(window.populateSessionDropdown=function(){originalPopulateSessionDropdown();const e=document.getElementById("chat-page-session-menu");if(e&&!e.querySelector(".session-search")){const t=document.createElement("div");t.className="session-search",t.innerHTML=`
        ...
        `}).join(""),e.querySelectorAll('input[type="checkbox"][data-agent]').forEach(r=>{r.addEventListener("change",()=>{const c=r.getAttribute("data-agent");if(!c)return;const d=getSidebarAgentsHiddenSet();r.checked?d.delete(c):d.add(c),setSidebarAgentsHiddenSet(d),applySidebarAgentsHidden()})}),t.onchange=()=>{const r=getSidebarAgentsPrefs();r.hideInactive=t.checked,r.inactivityMs=Number(n.value||0)||900*1e3,setSidebarAgentsPrefs(r),updateSidebarAgentActivityIndicators(window.availableSessions||[]),applySidebarAgentsHidden()},n.onchange=()=>{const r=getSidebarAgentsPrefs();r.hideInactive=t.checked,r.inactivityMs=Number(n.value||0)||900*1e3,setSidebarAgentsPrefs(r),updateSidebarAgentActivityIndicators(window.availableSessions||[]),applySidebarAgentsHidden()}}function resetSidebarAgentsOrder(){try{localStorage.removeItem(SIDEBAR_AGENTS_ORDER_KEY)}catch{}location.reload()}function setupSidebarAgentsManageButton(){const e=document.getElementById("sidebar-agents-manage-btn");e&&e.addEventListener("click",t=>{t.preventDefault(),t.stopPropagation(),openSidebarAgentsModal()})}const AVATAR_EXTENSIONS=["png","svg"];function resolveAvatarUrl(e){return e==="main"?"/avatars/solobot.png":`/avatars/${e}.png`}function agentDisplayName(e){const t=e.id||e.name,n=typeof AGENT_PERSONAS<"u"&&AGENT_PERSONAS[t];if(n)return`${n.name} (${n.role})`;if(e.isDefault)return"Halo (PA)";const s=e.name||e.id;return s.charAt(0).toUpperCase()+s.slice(1)}async function loadSidebarAgents(){const e=document.getElementById("sidebar-agents-list");if(e)try{const s=(await(await fetch("/api/agents")).json()).agents||[],a=s.some(i=>i.isDefault||i.id==="main")?s:[{id:"main",name:"main",emoji:"",isDefault:!0},...s];a.sort((i,r)=>i.isDefault?-1:r.isDefault?1:i.id.localeCompare(r.id)),e.innerHTML=a.map(i=>{const r=resolveAvatarUrl(i.id),c=agentDisplayName(i),d=i.emoji||"",l=(i.name||i.id).charAt(0).toUpperCase();return`
                <div class="sidebar-agent" data-agent="${i.id}">
                    ...
                    <span class="sidebar-item-text">${c}</span>
                </div>
            ...
            <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 10px;">
                <div style="flex: 1; min-width: 0;">
                    <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                        ...
                        <span style="font-weight: 650; font-size: 14px;">${escapeHtml(m)} ${escapeHtml(o)}</span>
